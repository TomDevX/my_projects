...
#tạo các hàm xét thắng thua và reset
def check_rows():
    """
    Kiểm tra kết quả theo dòng
    Trả về kí hiệu của bên thắng cuộc, nếu chưa có người thắng trả về None
    """
...
def check_diag():
    """
    Kiểm tra kết quả theo đường chéo
    Trả về kí hiệu của bên thắng cuộc, nếu chưa có người thắng trả về None
    """
    diag = [0, 4, 8, 2, 4, 6]
...
def check_tie():
    """
    Trả về True nếu hoà, False nếu ngược lại
    """
...
def check_win():
    """
        Trả về True và reset nếu đã có kết quả lượt chơi, False nếu chưa có kết quả
    """
    choose = ""
    player_win = check_rows()
    if player_win != None:
        choose = messagebox.yesno("Message", player_win + " win. Do you want to play again? ")
    else:
...
def reset():
    """
    Đặt lại các button về trạng thái ban đầu để chơi lại
    """
    i = 1
    for button in button_list:
...
def check_almost_win_diags():
    ...
    for i in range(0, 4, 3):
        ...
        if button_list[diag[i+1]]["text"] == button_list[diag[i]]["text"] and button_list[diag[i]]["text"] not in symbol:
...
def CPU():
    global turn, button_list, priority
    if turn == 1: #nếu là nước đi đầu tiên thì tuỳ chọn ô
        priority[0].config(text="O", bg="white", state="disable")
        priority.pop(0)
    else: #từ nước đi thứ hai, kiểm tra xem có bên nào sắp win chưa, nếu chưa có tiếp tục đi theo priority
        ...
        if almost_win != None:
            ...
            if almost_win != None:
                ...
                if almost_win != None:
                    almost_win.config(text="O", bg="white", state="disable")
                    priority.remove(almost_win)
                else: #nếu chưa có bên nào sắp win thì đi theo priority
                    priority[0].config(text="O", bg="white", state="disable")
                    priority.pop(0)
...
#lượt đi máy tính là O, người là X
def button1_click():
    global turn
    if mode == 2:
        if turn%2 == 0:
...
def click_one_player_mode():
    ...
    button_ok = tk.Button(text= "OK", command = button_ok_click)
...
def click_two_player_mode():
    ...
    one_player_mode.place_forget()
    two_player_mode.place_forget()
    global name_label1, name_entry1, name_label2, name_entry2, button_ok, mode
    mode = 2
    name_label1 = tk.Label(window, text = "Enter player1 name: ", font = ("Calibri", 15), bg= "#cc9900")
    name_label1.place(x = 100, y = 90)

    name_label2 = tk.Label(window, text = "Enter player2 name: ", font = ("Calibri", 15), bg= "#cc9900")
    name_label2.place(x = 100, y = 150)

    name_entry1 = tk.Entry(window, font = ("Arial", 10), justify = "center")
    name_entry1.place(x = 260, y = 90, width = 150, height = 30)

    name_entry2 = tk.Entry(window, font = ("Arial", 10), justify = "center")
    name_entry2.place(x = 260, y = 150, width = 150, height = 30)
...
one_player_mode = tk.Checkbutton(window, text = "one player mode", bg= "#cc9900", font=("Calibri Bold", 20), command = click_one_player_mode)
one_player_mode.place(x = 220, y = 120)

two_player_mode = tk.Checkbutton(window, text = "two player mode", bg= "#cc9900", font=("Calibri Bold", 20), command = click_two_player_mode)
two_player_mode.place(x = 220, y = 160)

img = tk.PhotoImage(file = "/Users/tomimac/Documents/Tom iMac/Sumato/Python 1.20/Bai Hoc/221014/tic-tac-toe.ppm")
picture = tk.Label(window, image = img)
picture.place(x = 150, y = 250, width = img.width(), height = img.height())
...
def button_ok_click():
    ...
    button_1.place(x=75, y=80, width=button_size, height=button_size)
    button_2.place(x=75 + button_size, y=80, width=button_size, height=button_size)
    button_3.place(x=75 + button_size*2, y=80, width=button_size, height=button_size)
    button_4.place(x=75, y=80 + button_size, width=button_size, height=button_size)
    button_5.place(x=75 + button_size, y=80 + button_size, width=button_size, height=button_size)
    button_6.place(x=75 + button_size*2, y=80 + button_size, width=button_size, height=button_size)
    button_7.place(x=75, y=80 + button_size*2, width=button_size, height=button_size)
    button_8.place(x=75 + button_size, y=80 + button_size*2, width=button_size, height=button_size)
    button_9.place(x=75 + button_size*2, y=80 + button_size*2, width=button_size, height=button_size)
